...
export default class StatsMonthlyArea extends Component {
    ...
    componentWillMount(){
        this.getDataReady(this.props)
    }

    componentWillReceiveProps(props){
        if (props.totalsByMonths.length !== this.props.totalsByMonths.length) {
            this.getDataReady(props)
        }
    }

    getDataReady(props){
        const {totalsByMonths} = props
    ...
    render(){
        const {categories, chartData} = this.state

        return <div className={css.main}>
            <AreaChart width={630} height={300} data={chartData} stackOffset="expand">

                <YAxis tickCount={3} tickFormatter={toPercent}/>
                <XAxis dataKey="month"/>
                ...
                <Tooltip
                    ...
                    itemSorter={(a, b) => a.dataKey - b.dataKey}
                />
            </AreaChart>
        </div>
    }
